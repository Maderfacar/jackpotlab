...
import type { ComboRequest } from '~/components/signals/SimilaritySection.vue'

const props = defineProps<{ state: AnalysisState, latestIssue: string, request?: ComboRequest | null }>()
...
// 相似段一鍵帶入：填值 → 立刻計算 → 捲到工具
watch(() => props.request, (req) => {
  if (!req) return
  gapTarget.value = req.gap
  valTarget.value = req.val
  tolerance.value = req.tolerance
  requestLabel.value = req.label
  run()
  nextTick(() => {
    ...
    <UCard>
      ...
      <div class="space-y-4">
        <div class="flex flex-wrap items-end gap-3">
          ...
          <label class="flex items-center gap-2 text-xs">
            <span class="text-muted">容差 ±</span>
            <UInput
              :model-value="String(tolerance)"
              type="number"
              size="sm"
              min="0"
              max="30"
              class="w-16"
              @change="(e: Event) => { const v = Number.parseInt((e.target as HTMLInputElement).value, 10); tolerance = Number.isFinite(v) && v >= 0 ? Math.min(30, v) : 0 }"
            />
          </label>
          <UButton
            color="primary"
