 ...
 *   1. 控制台
 *      - 第 1 行：歷史回顧顯示最近 n 期
 *      - 第 2 行：規則開關（扣紅框 / 選偏離 / 記錄最新數據）
 *      - 第 3 行：隔期顯示 0-9 各自開關
 *      - 第 4 行：規則參數（選偏離上下限、記錄最新首碼上限）
 *      - 第 5 行：圖例
 *   2. sticky 候選 section（避開頂部 UHeader）：下一期候選（待開獎）
 *   3. 歷史候選回顧列表（不 sticky）：每期一張卡、4 色標示
 *
 * 候選機制（按隔期分類處理、全程不合併、無 sort、無截斷）：
 *   raw = pre-T 該隔期 prizesBefore（sorted asc）
 *   控制台規則 = 三條獨立的 pool 過濾條件：
 *     扣紅框：上兩期共同號（連莊）移除（套用所有隔期）
 *     選偏離：保留 deviation ∈ [lower%, upper%]、超出範圍移除
 *     記錄最新數據：slot.recordValueBefore > n → 整支隔期排除
 *   通過過濾的全部號 = 主推（不再做 top-T 截斷）
 *
 * 4 色狀態：
 *   命中（主推 + 開出）→ 綠底 solid
...
type ExcludedReason = 'carryover' | 'deviation' | 'recordLatest'
...
function annotateInterval(
  ...
  // 防呆：v-model.number 中間狀態 (例如剛輸入 '-') 可能傳入非 number；
...
/**
 * 把 annotated items 包成 CandidateRow（給 UI 顯示）。
 * 通過過濾的全部為主推、無 sort 無截斷。
 */
function buildRowsFromAnnotated(
  ...
  for (const [j, items] of perIntervalItems) {
    ...
    for (const item of items) {
      ...
      // 展示順序：recordLatest → carryover → deviation
      ...
      const isRecommended = !excludedBy
  ...
  if (isPending) {
    ...
    for (const j of activeIntervals.value) {
      ...
      const { items, recordExcluded } = annotateInterval(
        j, raw, carryoverSet, recVal, numberStatsMap.value, rules, params
      )
        ...
        >
          <div class="flex items-baseline gap-2 flex-wrap text-[10px]">
            ...
            <span class="text-muted">
              原 {{ row.rawCount }} 顆 · 數值 {{ row.recordValue }} → 候選池 {{ row.poolSize }} · 主推 <span class="font-mono font-semibold text-emerald-600 dark:text-emerald-400">{{ row.recommendedCount }}</span>
            </span>
            <span
          ...
          <div class="text-[9px] text-muted">
            紅框 {{ row.removedByCarryover }} · 選偏離 {{ row.removedByDeviation }} · 記錄 {{ row.removedByRecord }}
          </div>
          <div
        ...
        <div class="space-y-2 text-xs">
          ...
          >
            <div class="flex items-baseline gap-2 flex-wrap text-[10px]">
              ...
              <span class="text-muted">
                原 {{ row.rawCount }} · 數值 {{ row.recordValue }} → 池 {{ row.poolSize }} · 主推 <span class="font-mono">{{ row.recommendedCount }}</span>
              </span>
              <span
                v-if="row.recordExcluded"
                class="text-[10px] text-amber-600 dark:text-amber-400"
