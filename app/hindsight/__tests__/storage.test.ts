...
describe('storage', () => {
  ...
  it('sweepStaleHindsightStorage 掃掉舊版本 key、留下當前版本', () => {
    ...
    ls.setItem(currentKey, JSON.stringify({ v: 1, gameId: 'lotto539', lastProcessedTerm: null, scorecards: {}, alerts: [], updatedAt: '' }))
    ls.setItem('jackpotlab-hindsight-lotto539-v0', 'old')
    ls.setItem('jackpotlab-hindsight-fake_game-v1', 'invalid')
    ...
    assert.equal(ls.getItem('jackpotlab-hindsight-lotto539-v0'), null, 'old version should be swept')
    assert.equal(ls.getItem('jackpotlab-hindsight-fake_game-v1'), null, 'invalid gameId should be swept')
