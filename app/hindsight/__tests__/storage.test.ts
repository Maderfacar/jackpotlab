...
describe('storage', () => {
  ...
  it('sweepStaleHindsightStorage 掃掉舊版本 key、留下當前版本', () => {
    ...
    ls.setItem(currentKey, JSON.stringify({ v: 4, gameId: 'lotto539', lastProcessedTerm: null, scorecards: {}, alerts: [], updatedAt: '' }))
    ls.setItem('jackpotlab-hindsight-lotto539-v3', 'old')
    ls.setItem('jackpotlab-hindsight-fake_game-v4', 'invalid')
    ...
    assert.equal(ls.getItem('jackpotlab-hindsight-fake_game-v4'), null, 'invalid gameId should be swept')
